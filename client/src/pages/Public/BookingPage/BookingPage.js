...
class BookingPage extends Component {
  ...
  jsPdfGenerator = () => {
    const { movie, cinema, selectedDate, selectedTime, QRCode, user } =
      this.props;
    const doc = new jsPDF();
    doc.setFont("helvetica");
    // doc.setFontType('bold');
    doc.setFontSize(22);
    doc.text(movie.title, 20, 15);
    doc.setFontSize(16);
    doc.text(cinema.name, 20, 20);
    doc.setFontSize(16);
    doc.text(user.name, 20, 30);
    doc.text(
      `Date: ${new Date(
        selectedDate
...
const mapStateToProps = (
  ...
  isAuth: authReducer.isAuthenticated,
  user: authReducer.user,
  movie: movieReducer.selectedMovie,
  cinema: cinemaReducer.selectedCinema,
  cinemas: cinemaReducer.cinemas,
  showtimes: showtimeReducer.showtimes.filter(
    (showtime) => showtime.movieId === ownProps.match.params.id
  ),
  reservations: reservationReducer.reservations,
  selectedSeats: checkoutReducer.selectedSeats,
  suggestedSeat: checkoutReducer.suggestedSeat,
  selectedCinema: checkoutReducer.selectedCinema,
  selectedDate: checkoutReducer.selectedDate,
  selectedTime: checkoutReducer.selectedTime,
  showLoginPopup: checkoutReducer.showLoginPopup,
  showInvitation: checkoutReducer.showInvitation,
  invitations: checkoutReducer.invitations,
  QRCode: checkoutReducer.QRCode,
  suggestedSeats: reservationReducer.suggestedSeats,
});
